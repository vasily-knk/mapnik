...
void export_layer()
{
    ...
    class_<layer>("Layer", "A Mapnik map layer.", init<std::string const&,optional<std::string const&> >(
        ...
        .def("visible", &layer::visible,
             "Return True if this layer's data is active and visible at a given scale_denonminator.\n"
             "\n"
             "Otherwise returns False.\n"
             "Accepts a scale_denonminator value as an integer or float input.\n"
             "Will return False if:\n"
             "\tscale_denonminator >= minimum_scale_denominator - 1e-6\n"
             "\tor:\n"
             "\tscale_denonminator < maximum_scale_denominator + 1e-6\n"
             "\n"
             "Usage:\n"
             ">>> from mapnik import Layer\n"
             ">>> lyr = Layer('My Layer','+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs')\n"
