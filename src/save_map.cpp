...
void serialize_layer( ptree & map_node, const layer & layer, bool explicit_defaults )
{
    ...
    {
        set_attr( layer_node, "minimum-scale-denominator", layer.minimum_scale_denominator() );
    }

    if ( layer.maximum_scale_denominator() != std::numeric_limits<double>::max() )
    {
        set_attr( layer_node, "maximum-scale-denominator", layer.maximum_scale_denominator() );
    }
